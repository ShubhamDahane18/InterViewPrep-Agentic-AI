...
### Rules:
- The valid sequence of sections is: ["interviewer_intro", "intro", "personal_fit", "behavioral", "role_fit", "end"].
- If current section is "interviewer_intro":
  - If candidate says they are ready → set section_name="intro".
...
- If current section is "end":
  ...
  - Example: "✅ Thank you! Your HR interview round is now complete. You may proceed to the next stage or request an analysis report for this round."
- If enforce_limit=true → ALWAYS move to the next section regardless of what the candidate says.  
  - If already in "role_fit", move to "end".
...
- response → a polite HR-style confirmation message to the candidate.  
"""),
    ("human", """
